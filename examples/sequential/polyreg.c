...
// f(x) = x³ -6x² + 3x + 7 + error
static void set_data(cml_matrix **x, cml_matrix **y)
{
    const fdouble err = prng->normal(prng, 0., 0.02);

    for (lgint i = 0; i < (*x)->m; i++)
    {
        const fdouble xi = prng->normal(prng, 0, 1);
        (*x)->set(x, i, 0, pow(xi, 3));
        (*x)->set(x, i, 1, -6 * pow(xi, 2));
        (*x)->set(x, i, 2, 3 * xi);
        (*x)->set(x, i, 3, 7 + err);
...
int main(void)
{
    ...
    cml_matrix *x = cml_matrix_zeros(100, 4);
